...
var resultData = require('./a.json');
var ary = resultData.data.results;
// console.log(ary);
// for (const item of ary) {
ary.forEach(item => {

    c.queue([{
        uri: `https://jobs.zhaopin.com/${item.number}.htm`,
        // The global callback won't be called
        callback: function (item,error, res, done, ) {
            ...
            if (error) {
                ...
                var $ = res.$;
                item.address = $('.add-txt').text();
                item.responsibility = $('.responsibility .pos-ul').text();
                dealResults.push(item);
            }
            if (dealResults.length === ary.length) {
                fs.writeFile("./b.json", JSON.stringify(dealResults), function (err) {
                    if (err) {
